...
export default function TradePage() {
  ...
  // Tracks whether a submit was attempted while the form was invalid —
  // gates the "what's missing" hint below so it doesn't nag before
  // anyone's touched the form. Council review 2026-08-23: the button
  // used to just go dead-silent when required fields were missing, with
  // zero indication of what to fix.
  const [attemptedSubmit, setAttemptedSubmit] = useState(false);

  const agencyValid = !!agencyName.trim();
  const emailValid = EMAIL_RE.test(email.trim());
  const canSubmit = agencyValid && emailValid;
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="trade-form-grid">
            ...
            <input
              ...
              style={{ width: '100%', padding: '12px 14px', borderRadius: 12, border: `1px solid ${attemptedSubmit && !agencyValid ? C.coral : C.sand}`, background: C.white, fontSize: 14, color: C.forest }}
            />
            <input
              type="text"
              value={contactPerson}
            ...
            <div className="trade-form-row">
              <input
                ...
                required
                placeholder="Email"
                style={{ padding: '12px 14px', borderRadius: 12, border: `1px solid ${attemptedSubmit && !emailValid ? C.coral : C.sand}`, background: C.white, fontSize: 14, color: C.forest }}
              />
              <input
                type="text"
                value={phoneOrLine}
            ...
            {/* Council review 2026-08-23: this button used to be
                HTML-`disabled` whenever canSubmit was false, which
                silently blocks the click/submit event entirely — there
                was no way for a customer to find out WHY nothing
                happened. It's now always clickable; handleSubmit itself
                checks canSubmit and, if invalid, only sets
                attemptedSubmit (see the hint text right below) instead
                of submitting. */}
            <button
              type="submit"
            ...
            {attemptedSubmit && !canSubmit && (
              <div style={{ marginTop: -6, fontSize: 12.5, color: C.coral, textAlign: 'center' }}>
                {!agencyValid && !emailValid
                  ? 'Please add your agency/guide name and a valid email address.'
                  : !agencyValid
                  ? 'Please add your agency or guide name.'
                  : 'Please add a valid email address (e.g. name@example.com).'}
              </div>
            )}
          </form>
        )}
